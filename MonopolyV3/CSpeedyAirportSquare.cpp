...
CSpeedyAirportSquare::CSpeedyAirportSquare(int number, string name, int cost, int rent) : CSquare(number, name)
{
	mAirportCost = cost;
	mAirportRent = rent;
}
...
// Destructor
CSpeedyAirportSquare::~CSpeedyAirportSquare()
{

}

// Getter. Returns airport cost
int CSpeedyAirportSquare::GetCost()
{
	return mAirportCost;
}

// Virtual function. Takes approrite action when player lands on Speedy Airport square
...
void CSpeedyAirportSquare::LandedOn(CPlayer* currentPlayer, CPlayer* otherPlayer)
{
